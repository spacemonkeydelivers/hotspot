...
class StubGenerator: public StubCodeGenerator {
 private:
  ...
  // Arguments:
  //
  //   X10 - call wrapper address     : address
  //   X11 - result                   : intptr_t*
  //   X12 - result type              : BasicType
  //   X13 - method                   : Method
  //   X14 - frame mgr entry point    : address
  //   X15 - parameter block          : intptr_t*
  //   X16 - parameter count in words : int
  //   X17 - thread                   : Thread*
  //
  address generate_call_stub(address& return_address) {
    ...
    // some sanity checks
    assert((frame::entry_frame_local_bytes % 16) == 0, "unaligned");

    Register Rarg_call_wrapper_addr        = X10_ARG0;
    Register Rarg_result_addr              = X11_ARG1;
    Register Rarg_result_type              = X12_ARG2;
    Register Rarg_method                   = X13_ARG3;
    Register Rarg_entry                    = X14_ARG4;
    Register Rarg_thread                   = X17_ARG7;

    Register Rtemp                         = X5_T0;
    Register Rtop_of_arguments_addr        = X6_T1;
    Register Rentryframe_fp                = X7_T2;
    ...
    //__ debug_start_trace();
    {
      ...
      Register Rarg_argument_addr          = X15_ARG5;
      Register Rarg_argument_count         = X16_ARG6;
      Register Rframe_alignment_in_bytes   = X18;
      Register Rargument_addr              = X19;
      Register Rargumentcopy_addr          = X20;
      Register Rargument_size_in_bytes     = X21;
      Register Rframe_size                 = X22;

      Label arguments_copied;

      // Push nonvolatile gprs.
      __ save_nonvolatile_gprs(X2_SP, -frame::spill_nonvolatiles_size);
      __ addi(X2_SP, X2_SP, -frame::spill_nonvolatiles_size);

      // Keep copy of our frame pointer (caller's SP).
      __ mv(Rentryframe_fp, X2_SP);

      BLOCK_COMMENT("Push entry frame including arguments");
      // Push ENTRY_FRAME including arguments:
      //
      //              [C frame]
      //      +-------alignment (optional)
      //      | Entry [Outgoing Java arguments]
      //      |       [Entry frame locals]
      //      | Frame [Min frame]
      //      +-------[Spilled non-volatile regs]
      //              ...

      // calculate frame size

      // unaligned size of arguments
      __ slli(Rargument_size_in_bytes, Rarg_argument_count, Interpreter::logStackElementSize);
      // arguments alignment (max 1 slot)
      // FIXME: use round_to() here
      __ andi(Rframe_alignment_in_bytes, Rarg_argument_count, 1);
      __ slli(Rframe_alignment_in_bytes,
              Rframe_alignment_in_bytes, Interpreter::logStackElementSize);
 
      // size = size of arguments + entry_frame_size
      __ addi(Rframe_size, Rargument_size_in_bytes, frame::entry_frame_local_bytes);
      // size += arguments alignment
      __ add(Rframe_size, Rframe_size, Rframe_alignment_in_bytes);

      // push entry frame
      __ push_frame(Rframe_size, Rtemp);

      // initialize call_stub locals (step 1)
      __ sd(Rarg_call_wrapper_addr,
             frame::entry_frame_call_wrapper_addr_index * wordSize, X8_FP);
      __ sd(Rarg_result_addr,
             frame::entry_frame_result_ptr_index * wordSize, X8_FP);
      __ sd(Rarg_result_type,
             frame::entry_frame_result_type_index * wordSize, X8_FP);
      // we will save arguments_tos_address later

      BLOCK_COMMENT("Copy Java arguments");
      // copy Java arguments

      // Calculate top_of_arguments_addr which will be tos register (not prepushed) later.
      __ add(Rtop_of_arguments_addr,
             X2_SP, Rframe_alignment_in_bytes);

      // any arguments to copy?
      __ beq(Rarg_argument_count, XZERO, arguments_copied);

      // prepare loop and copy arguments in reverse order
      {
        // let Rargumentcopy_addr point to last outgoing Java argument
        __ mv(Rargumentcopy_addr, Rtop_of_arguments_addr);

        // let Rargument_addr point to last incoming Java argument
        __ add(Rargument_addr, Rarg_argument_addr, Rargument_size_in_bytes);
        __ addi(Rargument_addr, Rargument_addr, -BytesPerWord);

        // now loop while Rarg_argument_count > 0 and copy arguments
        {
          Label next_argument;
          __ bind(next_argument);

          __ ld(Rtemp, 0, Rargument_addr);
          // argument_addr--;
          __ addi(Rargument_addr, Rargument_addr, -BytesPerWord);
          __ sd(Rtemp, 0, Rargumentcopy_addr);
          // argumentcopy_addr++;
          __ addi(Rargumentcopy_addr, Rargumentcopy_addr, BytesPerWord);
          // arg_argument_count--;
          __ addi(Rarg_argument_count, Rarg_argument_count, -1);

          __ blt(XZERO, Rarg_argument_count, next_argument);
        }
      }
...

    {
      ...
      Register Rnew_arg_entry = X28_T3;
      assert_different_registers(Rnew_arg_entry, Rtop_of_arguments_addr,
                                 Rarg_method, Rarg_thread);

      __ mv(Rnew_arg_entry, Rarg_entry);

      // Register state on entry to frame manager / native entry:
      //
      //   tos         -  intptr_t*    sender tos (prepushed) Lesp = (SP) + copied_arguments_offset - 8
      //   X23_method  -  Method
      //   X20_thread  -  JavaThread*

      // tos must point to last argument - element_size.
      const Register tos = X19_esp;
      __ addi(tos, Rtop_of_arguments_addr, -Interpreter::stackElementSize);

      // initialize call_stub locals (step 2)
      // now save tos as arguments_tos_address
      __ sd(tos, frame::entry_frame_args_tos_addr_index * wordSize, Rentryframe_fp);

      // load argument registers for call
      __ mv(X23_method, Rarg_method);
      __ mv(X20_thread, Rarg_thread);
      assert(tos != Rarg_method, "trashed Rarg_method");
      assert(tos != Rarg_thread && X23_method != Rarg_thread, "trashed r_arg_thread");
      ...
      __ mv(X24_tmp1, X2_SP);

      // Do a light-weight C-call here, Rnew_arg_entry holds the address
      // of the interpreter entry point (frame manager or native entry)
      // and save runtime-value of LR in return_address.
      assert(Rnew_arg_entry != tos && Rnew_arg_entry != X23_method && Rnew_arg_entry != X20_thread,
             "trashed Rnew_arg_entry");
      return_address = __ call_stub(Rnew_arg_entry);
    }

    {
      ...
      assert_different_registers(X10_RET0, Rarg_result_addr, Rarg_result_type);
      __ pop_frame();
      __ ld(Rarg_result_addr,
            frame::entry_frame_result_ptr_index * wordSize, X2_SP);
      __ ld(Rarg_result_type,
            frame::entry_frame_result_type_index * wordSize, X2_SP);

      // restore non-volatile registers
      __ addi(X2_SP, X2_SP, frame::spill_nonvolatiles_size);
      __ restore_nonvolatile_gprs(X2_SP, -frame::spill_nonvolatiles_size);
      ...
      __ addi(X5_T0, XZERO, T_OBJECT);
      __ beq(X5_T0, Rarg_result_type, ret_is_object);

      __ addi(X5_T0, XZERO, T_LONG);
      __ beq(X5_T0, Rarg_result_type, ret_is_long);

      __ addi(X5_T0, XZERO, T_FLOAT);
      __ beq(X5_T0, Rarg_result_type, ret_is_float);

      __ addi(X5_T0, XZERO, T_DOUBLE);
      __ beq(X5_T0, Rarg_result_type, ret_is_double);

      // default:
      __ sw(X10_RET0, 0, Rarg_result_addr);
      __ jr(X1_RA); // return to caller

      // case T_OBJECT:
      __ bind(ret_is_object);
      __ sd(X10_RET0, 0, Rarg_result_addr);
      __ jr(X1_RA); // return to caller
      ...
      __ fsw(F10_RET0, 0, Rarg_result_addr);
      __ jr(X1_RA); // return to caller

      // case T_DOUBLE:
      __ bind(ret_is_double);
      __ fsd(F10_RET0, 0, Rarg_result_addr);
      __ jr(X1_RA); // return to caller
    }
  ...
  address generate_catch_exception() {
    ...
    address start = __ pc();
    __ stop("StubRoutines::catch exception: Not Implemented");
    return start;
  /*
    StubCodeMark mark(this, "StubRoutines", "catch_exception");
  ...
  */
  ...
  address generate_forward_exception() {
    ...
    address start = __ pc();
    __ stop("StubRoutines::forward exception: Not Implemented");
    // TODO: Implement forward exceptions
/*
#if !defined(PRODUCT)
    if (VerifyOops) {
...
#define __ masm->
  ...
  address generate_throw_exception(const char* name, address runtime_entry, bool restore_saved_exception_pc,
    ...
    __ stop("StubRoutines::throw_exception: Not Implemented");
    return start;
/*
    ...
    RuntimeStub* stub =
      RuntimeStub::new_runtime_stub(name, &code,
                                    *//*frame_complete= (int)(frame_complete_pc - start)*//* 0,
...
#define __ _masm->
      ...
      case BarrierSet::CardTableExtension:
        {
          ...
          if (UseConcMarkSweepGC) {
            // TODO RISCV port: contribute optimization / requires shared changes
            __ release();
          }
  ...
  */
  ...
  /*
    ...
    address start = __ function_entry();
    __ unimplemented("StubRoutines::verify_oop", 95);  // TODO RISCV port
    return start;
  */
  }
  ...
  */
  ...
  void generate_arraycopy_stubs() {
    ...
    */
    ...
    // non-aligned disjoint versions
    StubRoutines::_jbyte_disjoint_arraycopy       = generate_unimplemented("jbyte_disjoint_arraycopy");
    StubRoutines::_jshort_disjoint_arraycopy      = generate_unimplemented("jshort_disjoint_arraycopy");
    StubRoutines::_jint_disjoint_arraycopy        = generate_unimplemented("jint_disjoint_arraycopy");
    StubRoutines::_jlong_disjoint_arraycopy       = generate_unimplemented("jlong_disjoint_arraycopy");
    StubRoutines::_oop_disjoint_arraycopy         = generate_unimplemented("oop_disjoint_arraycopy");
    StubRoutines::_oop_disjoint_arraycopy_uninit  = generate_unimplemented("oop_disjoint_arraycopy_uninit");

    // aligned disjoint versions
    StubRoutines::_arrayof_jbyte_disjoint_arraycopy      = generate_unimplemented("arrayof_jbyte_disjoint_arraycopy");
    StubRoutines::_arrayof_jshort_disjoint_arraycopy     = generate_unimplemented("arrayof_jshort_disjoint_arraycopy");
    StubRoutines::_arrayof_jint_disjoint_arraycopy       = generate_unimplemented("arrayof_jint_disjoint_arraycopy");
    StubRoutines::_arrayof_jlong_disjoint_arraycopy      = generate_unimplemented("arrayof_jlong_disjoint_arraycopy");
    StubRoutines::_arrayof_oop_disjoint_arraycopy        = generate_unimplemented("arrayof_oop_disjoint_arraycopy");
    StubRoutines::_arrayof_oop_disjoint_arraycopy_uninit = generate_unimplemented("oop_disjoint_arraycopy_uninit");

    // non-aligned conjoint versions
    StubRoutines::_jbyte_arraycopy      = generate_unimplemented("jbyte_arraycopy");
    StubRoutines::_jshort_arraycopy     = generate_unimplemented("jshort_arraycopy");
    StubRoutines::_jint_arraycopy       = generate_unimplemented("jint_arraycopy");
    StubRoutines::_jlong_arraycopy      = generate_unimplemented("jlong_arraycopy");
    StubRoutines::_oop_arraycopy        = generate_unimplemented("oop_arraycopy");
    StubRoutines::_oop_arraycopy_uninit = generate_unimplemented("oop_arraycopy_uninit");

    // aligned conjoint versions
    StubRoutines::_arrayof_jbyte_arraycopy      = generate_unimplemented("arrayof_jbyte_arraycopy");
    StubRoutines::_arrayof_jshort_arraycopy     = generate_unimplemented("arrayof_jshort_arraycopy");
    StubRoutines::_arrayof_jint_arraycopy       = generate_unimplemented("arrayof_jint_arraycopy");
    StubRoutines::_arrayof_jlong_arraycopy      = generate_unimplemented("arrayof_jlong_arraycopy");
    StubRoutines::_arrayof_oop_arraycopy        = generate_unimplemented("arrayof_oop_arraycopy");
    StubRoutines::_arrayof_oop_arraycopy_uninit = generate_unimplemented("arrayof_oop_arraycopy");

    // fill routines
    StubRoutines::_jbyte_fill          = generate_unimplemented( "jbyte_fill");
    StubRoutines::_jshort_fill         = generate_unimplemented( "jshort_fill");
    StubRoutines::_jint_fill           = generate_unimplemented( "jint_fill");
    StubRoutines::_arrayof_jbyte_fill  = generate_unimplemented("arrayof_jbyte_fill");
    StubRoutines::_arrayof_jshort_fill = generate_unimplemented("arrayof_jshort_fill");
    StubRoutines::_arrayof_jint_fill   = generate_unimplemented("arrayof_jint_fill");
  }

  address generate_unimplemented(const char* str) {
    address start = __ function_entry();
    __ unimplemented(str);
    return start;
  }
  ...
  void generate_safefetch(const char* name, int size, address* entry, address* fault_pc, address* continuation_pc) {
    ...
    *entry = __ function_entry();
    __ unimplemented("generate_safefetch");
    /*
