...
void InterpreterMacroAssembler::dbgtrace_gencode_post_impl(Register base,
                                                           int offset,
                                                           Register tmp,
                                                           uint64_t msg_id)
{
...
void InterpreterMacroAssembler::dbgtrace_gencode_post(Register thread_reg,
                                                      Register tmp,
                                                      const char* fmt, ...)
{
  char buffer[1024];
...
void InterpreterMacroAssembler::dispatch_next(TosState state, int bcp_incr) {
  // TODO: Check temp reg if OK
  Register bytecode = X7_T2;
  lbu(bytecode, bcp_incr, X18_bcp);
  if (bcp_incr != 0) 
    addi(X18_bcp, X18_bcp, bcp_incr);
...
void InterpreterMacroAssembler::dispatch_prolog(TosState state, int bcp_incr) {
  ...
  load_dispatch_table(X24_dispatch_addr, Interpreter::dispatch_table(state));

  slli(bytecode, bytecode, LogBytesPerWord);
  add(X24_dispatch_addr, X24_dispatch_addr, bytecode);
  ld(X24_dispatch_addr, 0, X24_dispatch_addr);
}
...
void InterpreterMacroAssembler::check_and_handle_earlyret(Register scratch_reg) {
  ...
  if (JvmtiExport::can_force_early_return()) {
    ...
    lwz(X10_ARG0, in_bytes(JvmtiThreadState::earlyret_tos_offset()), Rthr_state_addr);
...
void InterpreterMacroAssembler::load_earlyret_value(TosState state, Register Rscratch1) {
  ...
  const Register Rscratch2   = X5_T0;

  ld(RjvmtiState, in_bytes(JavaThread::jvmti_thread_state_offset()), X20_thread);

  switch (state) {
    case atos: ld(X21_tos, in_bytes(JvmtiThreadState::earlyret_oop_offset()), RjvmtiState);
               sd(XZERO, in_bytes(JvmtiThreadState::earlyret_oop_offset()), RjvmtiState);
               break;
    case ltos: ld(X21_tos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
               break;
    case btos: // fall through
    case ctos: // fall through
    case stos: // fall through
    case itos: lwu(X21_tos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
               break;
    case ftos: flw(F18_ftos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
               break;
    case dtos: fld(F18_ftos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
               break;
    case vtos: break;
  ...
  sd(XZERO, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
  // Set tos state field to illegal value.
  li(Rscratch2, ilgl);
  sw(Rscratch2, in_bytes(JvmtiThreadState::earlyret_tos_offset()), RjvmtiState);
}
...
void InterpreterMacroAssembler::load_dispatch_table(Register dst, address* table) {
  ...
  if (is_simm(table_offs, 12)) {
    addi(dst, X25_templateTableBase, (int)table_offs);
  } else {
    load_const(dst, table);
  }
}
...
#else
  assert_different_registers(bytecode, X6_T1);

  // Calc dispatch table address.
  load_dispatch_table(X6_T1, table);

  slli(X7_T2, bytecode, LogBytesPerWord);
  add(X7_T2, X7_T2, X6_T1);
  ld(X6_T1, 0, X7_T2);
...
void InterpreterMacroAssembler::load_receiver(Register Rparam_count, Register Rrecv_dst) {
  ...
  add(Rrecv_dst, Rrecv_dst, X19_esp);
  ld(Rrecv_dst, 0, Rrecv_dst);
}

// helpers for expression stack

void InterpreterMacroAssembler::pop_i(Register r) {
  lw(r, Interpreter::stackElementSize, X19_esp);
  addi(X19_esp, X19_esp, Interpreter::stackElementSize);
}

void InterpreterMacroAssembler::pop_ptr(Register r) {
  ld(r, Interpreter::stackElementSize, X19_esp);
  addi(X19_esp, X19_esp, Interpreter::stackElementSize);
}

void InterpreterMacroAssembler::pop_l(Register r) {
  ld(r, Interpreter::stackElementSize, X19_esp);
  // TODO: This is weird, check this later (why do we need 2 words for a long?)
  addi(X19_esp, X19_esp, 2 * Interpreter::stackElementSize);
}

void InterpreterMacroAssembler::pop_f(FloatRegister f) {
  flw(f, Interpreter::stackElementSize, X19_esp);
  addi(X19_esp, X19_esp, Interpreter::stackElementSize);
}

void InterpreterMacroAssembler::pop_d(FloatRegister f) {
  fld(f, Interpreter::stackElementSize, X19_esp);
  // TODO: This is weird, check this later (why do we need 2 words for a double?)
  addi(X19_esp, X19_esp, 2 * Interpreter::stackElementSize);
}
...
void InterpreterMacroAssembler::push_l(Register r) {
  sd(r, - Interpreter::stackElementSize, X19_esp);
  addi(X19_esp, X19_esp, - 2 * Interpreter::stackElementSize );
}
...
void InterpreterMacroAssembler::push_d(FloatRegister f)   {
  fsd(f, - Interpreter::stackElementSize, X19_esp);
  addi(X19_esp, X19_esp, - 2 * Interpreter::stackElementSize );
}

void InterpreterMacroAssembler::push_2ptrs(Register first, Register second) {
  sd(first, 0, X19_esp);
  sd(second, -Interpreter::stackElementSize, X19_esp);
  addi(X19_esp, X19_esp, - 2 * Interpreter::stackElementSize );
}
...
void InterpreterMacroAssembler::pop(TosState state) {
  ...
  verify_oop(X21_tos, state);
}

void InterpreterMacroAssembler::empty_expression_stack() {
  addi(X19_esp, X26_monitor, - Interpreter::stackElementSize);
}
...
void InterpreterMacroAssembler::get_2_byte_integer_at_bcp(int         bcp_offset,
  ...
  // Big Endian load off of BCP
  if (is_signed == Signed) {
    lb(Rdst, bcp_offset, X18_bcp);
  } else {
    lbu(Rdst, bcp_offset, X18_bcp);
  }
  lbu(Rscratch, bcp_offset + 1, X18_bcp);
  slli(Rdst, Rdst, 8);
  or_(Rdst, Rdst, Rscratch);
}
...
void InterpreterMacroAssembler::get_4_byte_integer_at_bcp(int         bcp_offset,
  ...
  for (int i = 1; i < 4; i++) {
    lbu(Rscratch, bcp_offset + i, X18_bcp);
    slli(Rdst, Rdst, 8);
    or_(Rdst, Rdst, Rscratch);
  }
}
...
void InterpreterMacroAssembler::get_cache_index_at_bcp(Register Rdst, int bcp_offset, size_t index_size) {
  ...
  if (index_size == sizeof(u2)) {
    lhu(Rdst, bcp_offset, X18_bcp);
  } else if (index_size == sizeof(u4)) {
    assert(EnableInvokeDynamic, "giant index used only for JSR 292");
    if (bcp_offset & 3) {
      li(Rdst, bcp_offset);
      add(Rdst, Rdst, X18_bcp);
      lw(Rdst, 0, Rdst);
    } else {
      lw(Rdst, bcp_offset, X18_bcp);
    }
    assert(ConstantPool::decode_invokedynamic_index(~123) == 123, "else change next line");
    xori(Rdst, Rdst, -1); // convert to plain index (invert)
...
void InterpreterMacroAssembler::get_cache_and_index_at_bcp(Register cache, int bcp_offset, size_t index_size) {
  ...
  add(cache, X27_constPoolCache, cache);
}
...
void InterpreterMacroAssembler::generate_stack_overflow_check_with_compare_and_throw(Register Rmem_frame_size, Register Rscratch1) {
  Label done;
  Register Rnext_SP = Rmem_frame_size;
  sub(Rnext_SP, X2_SP, Rmem_frame_size);
  ld(Rscratch1, thread_(stack_overflow_limit));
  blt(Rscratch1, Rnext_SP, done);

  // Load target address of the runtime stub.
  assert(StubRoutines::throw_StackOverflowError_entry() != NULL, "generated in wrong order");
  li(Rscratch1, (intptr_t) StubRoutines::throw_StackOverflowError_entry());
  jr(Rscratch1);

  /* TODO: Figure out if we need to restore: right now, not necessary
  // Restore caller_sp.
#ifdef ASSERT
  ld(Rscratch1, 0, X2_SP);
  ld(R0, 0, R21_sender_SP);
  cmpd(CCR0, R0, Rscratch1);
  asm_assert_eq("backlink", 0x547);
#endif // ASSERT
  mr(X2_SP, R21_sender_SP);
  */
...
void InterpreterMacroAssembler::index_check_without_pop(Register Rarray, Register Rindex, int index_shift, Register Rtmp, Register Rres) {
  ...
  */ /* Why doesn't PPC sign extend in integer arithmetic ops? I will.. */ /*
  ...
  // Index check
  lwu(Rlength, arrayOopDesc::length_offset_in_bytes(), Rarray);
  slli(RsxtIndex, RsxtIndex, index_shift);
  bltu(Rindex, Rlength, LnotOOR);
  // Index should be in X21_tos, array should be in X11_ARG1.
  mv(X21_tos, Rindex);
  mv(X11_ARG1, Rarray);
...
void InterpreterMacroAssembler::unlock_if_synchronized_method(TosState state,
  ...
  Label Lunlocked, Lno_unlock;
  {
    ...
    lwz(Raccess_flags, in_bytes(Method::access_flags_offset()), X23_method);
    li(R0, 0);
  ...
  bind(Lunlocked);
  {
    ...
    // Set up search loop: Calc num of iterations.
    {
      ...
      subf_(Riterations, X26_monitor, Rmonitor_base);
      ble(CCR0, Lno_unlock);
...
void InterpreterMacroAssembler::merge_frames(Register Rsender_sp, Register return_pc, Register Rscratch1, Register Rscratch2) {
  ...
  // Merge top frames.
  subf(Rscratch1, X2_SP, Rsender_sp); // top_frame_sp - SP
  stdux(Rscratch2, X2_SP, Rscratch1); // atomically set *(SP = top_frame_sp) = **SP
  */
}
...
void InterpreterMacroAssembler::lock_object(Register monitor, Register object) {
  ...
  if (UseHeavyMonitors) {
    ...
    sub(current_header, current_header, X2_SP);
...
void InterpreterMacroAssembler::call_from_interpreter(Register Rtarget_method, Register Rret_addr, Register Rscratch1, Register Rscratch2) {
  ...
  */
  ...
  {
    Label Lok;
    bne(Rtarget_addr, XZERO, Lok);
    stop("null entry point");
    bind(Lok);
  }
#endif // ASSERT
  save_interpreter_state();
  mv_if_needed(X23_method, Rtarget_method);
  mv(X1_RA, Rret_addr);
  jr(Rtarget_addr);
}
...
void InterpreterMacroAssembler::set_method_data_pointer_for_bcp() {
  ...
  Label get_continue;
  ld(X9_mdx, in_bytes(Method::method_data_offset()), X23_method);
  test_method_data_pointer(get_continue);
  call_VM_leaf(CAST_FROM_FN_PTR(address, InterpreterRuntime::bcp_to_di), X23_method, X18_bcp);

  addi(X9_mdx, X9_mdx, in_bytes(MethodData::data_offset()));
  add(X9_mdx, X9_mdx, X10_RET0);
  bind(get_continue);
}
...
void InterpreterMacroAssembler::test_method_data_pointer(Label& zero_continue) {
  assert(ProfileInterpreter, "must be profiling interpreter");
  beq(X9_mdx, XZERO, zero_continue);
}
...
void InterpreterMacroAssembler::verify_method_data_pointer() {
  ...
  ld(R12_scratch2, in_bytes(Method::const_offset()), X23_method);
  addi(R11_scratch1, R11_scratch1, in_bytes(ConstMethod::codes_offset()));
  add(R11_scratch1, R12_scratch2, R12_scratch2);
  cmpd(CCR0, R11_scratch1, X18_bcp);
  beq(CCR0, verify_continue);

  call_VM_leaf(CAST_FROM_FN_PTR(address, InterpreterRuntime::verify_mdp ), X23_method, X18_bcp, R28_mdx);
...
void InterpreterMacroAssembler::test_backedge_count_for_osr(Register backedge_count, Register branch_bcp, Register Rtmp) {
  ...
  set_top_ijava_frame_at_SP_as_last_Java_frame(X2_SP, R11_scratch1);
...
void InterpreterMacroAssembler::profile_return_type(Register ret, Register tmp1, Register tmp2) {
  ...
  if (ProfileInterpreter && MethodData::profile_return()) {
    ...
    if (MethodData::profile_return_jsr292_only()) {
      ...
      // length.
      lbz(tmp1, 0, X18_bcp);
      lbz(tmp2, Method::intrinsic_id_offset_in_bytes(), X23_method);
...
void InterpreterMacroAssembler::add_monitor_to_stack(bool stack_is_empty, Register Rtemp1, Register Rtemp2) {
  ...
  std(X2_SP, _ijava_state_neg(top_frame_sp), esp); // esp contains fp
  ...
  if (!stack_is_empty) {
    ...
    addi(esp, X19_esp, Interpreter::stackElementSize); // Point to first element (pre-pushed stack).
    subf(n_slots, esp, X26_monitor);
  ...
  addi(X19_esp, X19_esp, -monitor_size);
  addi(X26_monitor, X26_monitor, -monitor_size);
...
void InterpreterMacroAssembler::load_local_int(Register Rdst_value, Register Rdst_address, Register Rindex) {
  ...
  sub(Rdst_address, X22_locals, Rdst_address);
  lwu(Rdst_value, 0, Rdst_address);
}
...
void InterpreterMacroAssembler::call_VM(Register oop_result, address entry_point, bool check_exceptions) {
  ...
  restore_interpreter_state(true);

  //TODO: Handle Jvmti cases:
  /*
...
void InterpreterMacroAssembler::call_VM(Register oop_result, address entry_point, Register arg_1, Register arg_2, Register arg_3, bool check_exceptions) {
  ...
  assert(arg_2 != X11_ARG1, "smashed argument");
  mv_if_needed(X12_ARG2, arg_2);
  assert(arg_3 != X11_ARG1 && arg_3 != X12_ARG2, "smashed argument");
  mv_if_needed(X13_ARG3, arg_3);
  call_VM(oop_result, entry_point, check_exceptions);
}

void InterpreterMacroAssembler::save_interpreter_state() {
  sd(X18_bcp,            frame::interpreter_frame_Rbcp_index       *frame::frame_elem_size, X8_FP);             
  sd(X23_method,         frame::interpreter_frame_Rmethod_index    *frame::frame_elem_size, X8_FP);             
  sd(X22_locals,         frame::interpreter_frame_Rlocals_index    *frame::frame_elem_size, X8_FP);             
  sd(X26_monitor,        frame::interpreter_frame_Rmonitors_index  *frame::frame_elem_size, X8_FP);             
  sd(X27_constPoolCache, frame::interpreter_frame_RcpoolCache_index*frame::frame_elem_size, X8_FP);             
  sd(X9_mdx,             frame::interpreter_frame_Rmdx_index       *frame::frame_elem_size, X8_FP);             
  sd(X19_esp,            frame::interpreter_frame_Resp_index       *frame::frame_elem_size, X8_FP);
}

void InterpreterMacroAssembler::restore_interpreter_state(bool bcp_and_mdx_only) {
  ld(X18_bcp, frame::interpreter_frame_Rbcp_index *frame::frame_elem_size, X8_FP); // Changed by VM code (exception).
  if (ProfileInterpreter) { 
    ld(X9_mdx, frame::interpreter_frame_Rmdx_index*frame::frame_elem_size, X8_FP);
  }
  if (!bcp_and_mdx_only) {
    ld(X23_method,         frame::interpreter_frame_Rmethod_index    *frame::frame_elem_size, X8_FP);             
    ld(X22_locals,         frame::interpreter_frame_Rlocals_index    *frame::frame_elem_size, X8_FP);             
    ld(X26_monitor,        frame::interpreter_frame_Rmonitors_index  *frame::frame_elem_size, X8_FP);             
    ld(X27_constPoolCache, frame::interpreter_frame_RcpoolCache_index*frame::frame_elem_size, X8_FP);             
    ld(X19_esp,            frame::interpreter_frame_Resp_index       *frame::frame_elem_size, X8_FP);
  }
#ifdef ASSERT
  // TODO: Add frame safety check
  /*
  {
    Label Lok;
...
void InterpreterMacroAssembler::verify_oop_or_return_address(Register reg, Register Rtmp) {
  ...
  // THis is a rotation instruction, WTF?
  ...
  save_volatile_gprs(X2_SP, -nbytes_save); // except R0
  push_frame_reg_args(nbytes_save, Rtmp);

  load_const(Rtmp, fd);
  mv_if_needed(X11_ARG1, reg);
  mv(X10_ARG0, X23_method);
  call_c(Rtmp); // call C

  pop_frame();
  restore_volatile_gprs(X2_SP, -nbytes_save); // except R0
  j(skip);
...
void InterpreterMacroAssembler::push_interpreter_frame(Register top_frame_size, Register parent_frame_resize,
  ...
  ld(tmp1, _top_ijava_frame_abi(frame_manager_lr), X2_SP);
  mr(tmp2*/ /*top_frame_sp*/ /*, X2_SP);
  // Move initial_caller_sp.
  ld(tmp4, _top_ijava_frame_abi(initial_caller_sp), X2_SP);
  ...
  std(tmp1, _abi(lr), X2_SP);
  // Set top_frame_sp info for new parent frame.
  std(tmp2, _parent_ijava_frame_abi(top_frame_sp), X2_SP);
  std(tmp4, _parent_ijava_frame_abi(initial_caller_sp), X2_SP);

  // Push new TOP_IJAVA_FRAME.
  push_frame(top_frame_size, tmp2);

  get_PC_trash_LR(tmp3);
  std(tmp3, _top_ijava_frame_abi(frame_manager_lr), X2_SP);
  // Used for non-initial callers by unextended_sp().
  std(X2_SP, _top_ijava_frame_abi(initial_caller_sp), X2_SP);
  */
}
...
void InterpreterMacroAssembler::pop_interpreter_frame(Register tmp1, Register tmp2, Register tmp3, Register tmp4) {
  ...
  ld(tmp1*/ /*caller's sp*/ /*, _abi(callers_sp), X2_SP);
  ...
  // Merge top frame.
  std(tmp2, _abi(callers_sp), X2_SP);
  ...
  std(tmp4, _top_ijava_frame_abi(initial_caller_sp), X2_SP);

  // Store the top-frame stack-pointer for c2i adapters.
  std(X2_SP, _top_ijava_frame_abi(top_frame_sp), X2_SP);
  */
}
...
void InterpreterMacroAssembler::pop_interpreter_frame_to_state(Register state, Register tmp1, Register tmp2, Register tmp3) {
  ...
  // Merge top frames.
  std(tmp1, _abi(callers_sp), X2_SP);
...
void InterpreterMacroAssembler::resize_frame_to_initial_caller(Register tmp1, Register tmp2) {
  unimplemented(__func__);
  /*
  ld(tmp1, _parent_ijava_frame_abi(initial_caller_sp), X2_SP);
  ld(tmp2, _parent_ijava_frame_abi(callers_sp), X2_SP);
  std(tmp2, _parent_ijava_frame_abi(callers_sp), tmp1); // Fix back chain ...
  mr(X2_SP, tmp1); // ... and resize to initial caller.
  */
}
