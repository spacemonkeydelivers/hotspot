...
#ifndef CPU_RISCV_VM_INTERP_MASM_RISCV_64_HPP
#define CPU_RISCV_VM_INTERP_MASM_RISCV_64_HPP
...
#define thread_(field_name) in_bytes(JavaThread::field_name ## _offset()), X20_thread
#define method_(field_name) in_bytes(Method::field_name ## _offset()), X23_method
...
#define prev_state_(field_name)  in_bytes(byte_offset_of(BytecodeInterpreter, field_name)), R15_prev_state
  ...
  void pop_i(     Register r = X21_tos);
  void pop_ptr(   Register r = X21_tos);
  void pop_l(     Register r = X21_tos);
  void pop_f(FloatRegister f = F18_ftos);
  void pop_d(FloatRegister f = F18_ftos );

  void push_i(     Register r = X21_tos);
  void push_ptr(   Register r = X21_tos);
  void push_l(     Register r = X21_tos);
  void push_f(FloatRegister f = F18_ftos );
  void push_d(FloatRegister f = F18_ftos);

  void push_2ptrs(Register first, Register second);

  void push_l_pop_d(Register l = X21_tos, FloatRegister d = F18_ftos);
  void push_d_pop_l(FloatRegister d = F18_ftos, Register l = X21_tos);
